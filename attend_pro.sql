...
values(to_char(sysdate,'yyyy/mm/dd'),44,to_char(sysdate,'yyyy/mm/dd hh24:mi:ss'),'W0');
...
select a.* ,GET_CODE_VALUE(a.ATT_CODE) ATT_STATE
from tbl_attend a
where to_char(attend_date, 'yyyy/mm/dd') = to_char(sysdate, 'yyyy/mm/dd') and emp_no = 4;  



--오늘 출퇴근한 전체 사원의  상태 업데이트 (지각이면 지각으로 코드값 처리) 
 UPDATE TBL_ATTEND OLD_TBL                          
