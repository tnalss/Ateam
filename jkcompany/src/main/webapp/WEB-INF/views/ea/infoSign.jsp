			...
			<div class="col-12 col-lg-9">
				...
				<c:forEach items="${info_list}" var="list">
                	<div class="card text-center col-3">
                    	...
                    	<div class="card-footer">
 							<c:choose>
 								<c:when test="${list.ea_receiver eq login.emp_no && list.ea_r_statuas eq '미결재' }">
 									<a class="btn icon-btn btn-primary approved" href="#">결재</a>
 									<input type="hidden" value="${list.ea_receiver}">	
 								...
 								<c:otherwise>
 									<p><strong>${list.ea_r_statuas}</strong><p>
 								</c:otherwise>
 							</c:choose>
...
$('.approved').click(function(){
	var div = $(this).parent('div');
	 ...
	 $.ajax({
		type:"POST",
		url:"update_statuas.ea",
		data:{ea_r_statuas: "E7" ,
			  ea_receiver: $(this).siblings('input').val(),
			  ea_num: ${map["ea_num"]},
			  total: ${info_list.size()}
			},
		dataType:"json",
		success: function(data){
			...
			if(data){
				div.empty();
				div.html("<img alt='결재완료' src='assets/img/ea_stamp_approved.png'>");
